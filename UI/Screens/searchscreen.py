...
class SearchScreen(Screen):
    ...
    def get_layout(self):
        ...
        self.search_line_edit_host.returnPressed.connect(lambda: self.search(self.search_line_edit_host.text(), 
                                                                        ...
                                                                        'host Name' if self.search_clusternodes_check.currentText() == 'Hosts' else 'Host/Listener Name') )
        ...
        label40.setStyleSheet("background-color: {};".format(config.bar_colour()))
    ...
    def search(self, text, standaloneb, alwaysonb, failoverb, hostnameb, instancenameb, clusternameb, hoststateo, serverstateo, clusternodeo, text2='', text3='', order_by=None):
        ...
        if clusternodeo == 'Instances':
            ...
            if ( not(standaloneb) and not(alwaysonb) and not(failoverb) ) or ( not(hostnameb) and not(instancenameb) and not(clusternameb) ):
                ...
                sql_results = sqlcommands.get_all_servers_like(servertypes, hoststate, serverstate, hostname, instancename, clustername, order_by)
        ...
        # Cluster node search
            ...
            sql_results = sqlcommands.get_all_hosts_like(text, hoststate, order_by)
        ...
        for i, data in enumerate(object_data):
            ...
            for key in data.keys():
                ...
                if key in headers_ordered:
                    ...
                    # Special columns
                    if key in ['Host/Listener Name', 'host Name', 'Windows Cluster Name', 'Instance Name', 'Server Type']:
    ...
    def button_open_cluster(self, source, data):
        def temp():
            ...
            '''
            self.master.add_popup(ClusterDetailsScreen(self.master, 'Cluster Details Screen', (source, data)))
        return temp
    ...
    def button_open_instance(self, source, data):
        def temp():
            ...
            returns:-
                None : Creates a popup with the info on
            '''
            self.master.add_popup(SQLServerDetailsScreen(self.master, 'SQL Server Details', (source, data)))
        return temp
    ...
    def button_open_host(self, source, data):
        def temp():
            ...
            returns:-
                None : Creates a popup with the info on
            '''
            self.master.add_popup(HostDetailsScreen(self.master, 'Host Details', (source, data)))
        return temp
    ...
    def button_open_ag(self, source, data):
        def temp():
            ...
            returns:-
                None : Creates a popup with the info on
            '''
            self.master.add_popup(AGDetailsScreen(self.master, 'AG Details', (source, data)))
        return temp
    ...
    def button_search_options(self, text):
        ...
        for i in values:
            ...
            if type(i) == QCheckBox:
                i.setChecked(False) if self.search_clusternodes_check.currentText() == 'Hosts' else i.setChecked(True)
    ...
    def button_order_by(self, order_by):
        def temp():
            self.search(self.search_line_edit_host.text(), 
                self.view_standalone_check.isChecked(), 
                self.view_alwayson_check.isChecked(), 
                self.view_failover_check.isChecked(),
                True,
